...
print("Тест линейного объектного S-Фильтра с контейнером SQLite")
print("")
...
for oid in book.Oids("sqlite").items:
	book.Oid(oid)
	book.DeleteObject("sqlite")

total_sum : int = 0

for index in range(50):
	book.Oid(f"Книга-{index:02d}")
	book.RegisterObject("sqlite")
	book.f_author.FromString("sqlite", f"Автор-{index}")
	book.f_year.FromInteger("sqlite", 1900 + index)
	book.f_price.FromInteger("sqlite", 10 + index)
	book.f_exist.FromBoolean("sqlite", index % 2 == 0)
...
result = filter_book.Capture("sqlite").code == RESULT_OK
print(f"{'[+]' if result else '[ ]'} Захват данных из контейнера SQLite")
