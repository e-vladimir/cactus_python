...
print("Тест линейного 1D-Фильтра с контейнером PostgreSQL")
print("")
...
# for oid in book.Oids("postgresql").items:
# 	book.Oid(oid)
# 	book.DeleteObject("postgresql")
...
for index in range(50):
	...
	# book.f_author.FromString("postgresql", f"Автор-{index}")
	# book.f_year.FromInteger("postgresql", 1900 + index)
	# book.f_price.FromInteger("postgresql", 10 + index)
	# book.f_exist.FromBoolean("postgresql", index % 2 == 0)
...
result = filter_book.Capture("postgresql").code == RESULT_OK
print(f"{'[+]' if result else '[ ]'} Захват данных из контейнера PostgreSQL")
...
filter_book.Capture("postgresql")
