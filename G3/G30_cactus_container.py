...
class C30_Container(C20_MetaFrame):
	...
	def DeleteSCell(self, cell: T20_StructCell) -> T21_StructResult_StructCell:
		""" Удаление S-Ячейки """
		return T21_StructResult_StructCell(code=CODES_COMPLETION.COMPLETED, subcodes=[CODES_PROCESSING.SKIP], data=cell)
	...
	def DCutRange(self, cell: T21_StructRange) -> T21_StructResult_StructRange:
		...
		return T21_StructResult_StructRange(code=CODES_COMPLETION.COMPLETED, subcodes=[CODES_PROCESSING.SKIP], data=cell)

	def DCuts(self, cell: T21_StructRange) -> T21_StructResult_List:
		""" Запрос списка CUT """
		return T21_StructResult_List(code=CODES_COMPLETION.COMPLETED, subcodes=[CODES_PROCESSING.SKIP], data=[])

	# ПРОВЕРКА ВИДА КОНТЕЙНЕРА
	def Type_RAM(self) -> T21_StructResult_Bool:
		""" Проверка вида контейнера: RAM """
		return T21_StructResult_Bool(code=CODES_COMPLETION.COMPLETED, subcodes=[], data=self._container_type == CONTAINERS.CONTAINER_RAM)

	def Type_SQLite(self) -> T21_StructResult_Bool:
		""" Проверка вида контейнера: SQLite """
		return T21_StructResult_Bool(code=CODES_COMPLETION.COMPLETED, subcodes=[], data=self._container_type == CONTAINERS.CONTAINER_SQLITE)

	def Type_PostgreSQL(self) -> T21_StructResult_Bool:
		""" Проверка вида контейнера: PostgreSQL """
		return T21_StructResult_Bool(code=CODES_COMPLETION.COMPLETED, subcodes=[], data=self._container_type == CONTAINERS.CONTAINER_POSTGRESQL)
